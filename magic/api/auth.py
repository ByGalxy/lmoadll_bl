...
def register_api():
    ...
    if email not in verification_codes:
        ...
        return response_handler.custom_error_response("验证码不存在或已过期喵喵")
    
    code_data = verification_codes[email]
    
    if int(time.time()) > code_data['expires_at']:
        del verification_codes[email]
        return response_handler.custom_error_response("验证码已过期喵喵")
    
    if not VerifyPassword(code_data['hash'], code):
        return response_handler.custom_error_response("验证码错误喵喵")
    
    if code_data['hash'] != code_salt:
        return response_handler.custom_error_response("验证码哈希不匹配喵喵")
